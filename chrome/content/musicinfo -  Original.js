...
function getBpmRangeMinMax(){   
	var list = LibraryUtils.mainLibrary;  
	var returnList = new Array();
	var genres = list.getDistinctValuesForProperty(SBProperties.bpm);
	while (genres.hasMore()) {  
		var value = genres.getNext();
...
function UR_Start() 
{
	...
	// Set new time
	//var text = document.getElementById("instructors-musicinfo-clock");
 	//text.setAttribute("value", UR_Indhold);
 	
 	// Update track position
 	//var position = SB_NewDataRemote( "metadata.position.str", null );
 	//var text = document.getElementById("instructors-musicinfo-trackInfo");
 	//text.setAttribute("value", position.stringValue);
...
{	
	...
	if(gUpdatedUI){	
		...
		gUpdateTimer = setTimeout("updatePlaylistUI();",100);	
	}	
	else{
...
{		
	...
	gMM.sequencer.playView(mediaList.createView(),playIndex);	
	gMM.sequencer.next();
...
{		
	...
	var minRating = getMinRating();	
	var items = LibraryUtils.mainLibrary.getItemsByProperty(SBProperties.genre, genre);	
	var itemEnum = items.enumerate(); 
	
	// Loop tracks and find matches
	var list = loopRecommendedTracks(itemEnum,bpmMin,bpmMax,minRating);
	
	// Make sure list has always 10 tracks
	var numTries = 0;	
	while (list.length < 10 && numTries < 10){
